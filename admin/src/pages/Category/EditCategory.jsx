...
const EditCategory = () => {
   ...
   console.log('editediddata', data);

   useEffect(() => {
      if (res?.data) {
          // Set date as a dayjs object for the DatePicker
          setData({
              ...res.data,
              date: dayjs(res.data.date), // Convert the date string into a dayjs object
          });
      }
  }, [res]);

   const handleDateChange = (date) => {
      setData(prev => ({ ...prev, date })); // Set the dayjs object directly
  };
   ...
   const handleDelete = () => {
      deleteCategory(data)
         .then((res) => {
            if (res) {
               toast.success(res?.message ?? "events deleted Successfully");
               navigate('/events')
            }
         })
         .catch((err) => {
   ...
   const handleSubmit = () => {
      try {
         ...
         editCategory(formData)
            .then((res) => {
               if (res) {
                  toast.success(res?.message ?? "events added Successfully");
                  navigate('/events')
               }
            })
            .catch((err) => {
               ...
               <Grid item xs={12}>
                  <LocalizationProvider dateAdapter={AdapterDayjs}>
                     <DatePicker
                        ...
                        onChange={handleDateChange} // Use handleDateChange for date updates
                        renderInput={(params) => <Input fullWidth {...params} />} // Ensure it uses Input for styling
                     />
                  </LocalizationProvider>
               </Grid>
               <Grid item xs={12}>
